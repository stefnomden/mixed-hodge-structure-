...
function (phi::FunctionFieldHomWithMHS)(f::AbstractAlgebra.Generic.AbsSimpleFunctionFieldElem{
  ...
  return sum([
    sum([a * x_im^(j-1) for (j,a) in enumerate(coefficients(b))], init = zero(x_im)) * 
    y_im^(i-1) for (i,b) in enumerate(coefficients(numerator(f)))], init = zero(y_im)
  ) // sum([a * x_im^(j-1) for (j,a) in enumerate(coefficients(denominator(f)))], init=zero(x_im))

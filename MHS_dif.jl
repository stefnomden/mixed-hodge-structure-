...
function mixed_hodge_structure(
  ...
  D_inf = [a for a in D if (length(a) == 3 && iszero(a[end]))]
  D_fin = [
    [a for a in D if length(a) == 2];
    [(a[1]//a[end], a[2]//a[end]) for a in D if (length(a) == 3 && !iszero(a[end]))]
  ]

  E_inf = [a for a in E if (length(a) == 3 && iszero(a[end]))]
  if !iszero(length(E_inf))
    E_fin = [
      [a for a in E if length(a) == 2];
      [(a[1]//a[end], a[2]//a[end]) for a in E if (length(a) == 3 && !iszero(a[end]))]
    ]

    alpha = zero(QQBar)
    x_E_fin = [a[1] for a in [E_fin; E_inf]]
    while (alpha in x_E_fin)
      alpha += one(QQBar)
    end
    println(alpha)
    coord_change = P -> (
      length(P) == 3 ? [zero(QQBar), P[2] * inv(P[1] - alpha)] : [inv(P[1] - alpha), P[2] * inv(P[1] - alpha)]
    )

    changed_E = Vector{Vector{QQBarFieldElem}}([coord_change(P) for P in [E_fin; E_inf]])
    changed_D = Vector{Vector{QQBarFieldElem}}([coord_change(P) for P in [D_fin; D_inf]])

    _, (z,y) = polynomial_ring(base_ring(P), [:z,:y])
    wan = one(base_ring(P))
    n = total_degree(P)
    changed_P = sum(f(wan,y) * z^(n - total_degree(f)) for f in Hecke.terms(P))
    
    return mixed_hodge_structure(changed_P; D = changed_D, E = changed_E)
  end
  ...
  HS.D = D_fin
  HS.D_inf = [a[1:end-1] for a in D_inf]
  ...
  if K == rational_field()
    ...
    for s in gens(K)
      ...
      j = findfirst(alpha -> overlaps(i(s), C(alpha)), candidates)
